...
// Load real users from Firestore for swipe
async function loadRealUsers(currentUid) {
  try {
    ...
    return data.documents
      .map(doc => {
        ...
        return {
          id: uid,
          name: f.name?.stringValue || "Moun",
          age: parseInt(f.age?.integerValue || f.age?.stringValue || 25),
          city: f.city?.stringValue || "",
          country: f.country?.stringValue || "",
          bio: f.bio?.stringValue || "",
          photoUrl: f.photoUrl?.stringValue || null,
          photos: (f.photos?.arrayValue?.values || []).map(v => v.stringValue),
          interests: (f.interests?.arrayValue?.values || []).map(v => v.stringValue),
      ...
      .filter(u => u && u.name && (u.photoUrl || u.photos?.length > 0));
  } catch(e) { return []; }
}
...
export default function TiKeke() {
  ...
  useEffect(() => {
    async function loadSession() {
      ...
      if (saved) {
        ...
        setUser(localData);
        if (localData.plan && localData.plan !== "free") {
  ...
  function confirmPay() {
    ...
    setTimeout(() => {
      setPaying(false);
      const newPlan = selectedPlan.key;
      setPlan(newPlan);
      setPayStep("success");
      ...
      if (updated.uid) saveUserProfile(updated);
    }, 1800);
  }
            ...
            <div style={{ width:"100%", maxWidth:380, height:480, position:"relative", marginBottom:24 }}>
              {cards.length === 0 ? (
                ...
                <div key={p.id} style={{
                  ...
                  <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", fontSize:110, background:`linear-gradient(160deg,${p.color}15,${p.color}35)`, overflow:"hidden", position:"relative" }}>
                    {(p.photoUrl || (p.photos && p.photos[0]))
                      ? <img src={p.photoUrl || p.photos[0]} alt={p.name} style={{ width:"100%", height:"100%", objectFit:"cover", position:"absolute", inset:0 }} />
                      : p.emoji
                    }
                  </div>
